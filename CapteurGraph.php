<!--page préparant l'affichage du graphqiue journalier, selelction du capteur et de la date. -->
...
die ($e->getMessage()) ;
 ...
 // $donnee= $bdd->prepare(" SELECT valeur, time(date_heure) AS timeH FROM releve2 INNER JOIN capteur ON capteur.idCapteur= releve2.idCapteur WHERE capteur.nom = 'Tfour1' AND DATE( date_heure) = '2020-05-27'   ");
 ...
 $donnee->execute($param); // requette sql qui permet de prendre se que l'on veux 
...
?>
 ...
 <form action="CapteurGraph2.php" class="form-inline" method="post">
  <label for="start">Sélenctionner une date de visionage:</label>

  <input type="date" id="start" name="dateSelect"
       value= ""
...
<!--courbe journalière en fonction de la date et du capteur   Réutilisation de chart.js voir graphique pour les détails-->
...
<script> 
    ...
    const chart = new Chart(ctx, {
        type: 'line',
        data: {
            labels:<?php  echo json_encode($heure); ?>, // on recupere les parametres de l'heure en haut 

            datasets: [{
                label: "instant-T",
                borderColor: 'rgba(100, 30, 22, 0.9)', // couleur du trait 
